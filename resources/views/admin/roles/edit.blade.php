<x-app-layout>
    ...
    <div class="flex">
        ...
        <div class="container mx-auto mt-4">
            <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="bg-white p-6 shadow-md rounded-md">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label for="name" class="block text-sm font-medium text-gray-700">Role Name</label>
                    <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md" required>
                    @error('name')
